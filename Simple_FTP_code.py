...
def ftp_command(ftp):
    while True:  # Run until 'exit' command is received from user            #### TAKE COMMAND (example: put SomeTextFile.txt) ####
        ...
        if commands[0] == 'cd':                                               #### CHANGE DIRECTORY (REMOTE) ####
            try:
                ftp.cwd(commands[1])
        ...
        elif commands[0] == 'get':                                           #### DOWNLOAD A FILE  ####
            try:
        ...
        elif commands[0] == 'ls':                                           #### PRINT REMOTE DIRECTORY ####
            print('Directory of current remote file', ftp.pwd())
            ftp.dir()

        elif commands[0] == 'lslocal':                                      #### PRINT LOCAL DIRECTORY ####
            print('Directory of current local file: \n')
            for i in os.listdir():
                print("Local file ---> " + i)


        elif commands[0] == 'put':                                         #### UPLOAD A FILE ###
            uploadfile = open(commands[1], 'rb')
            ftp.storbinary('STOR ' + commands[1], uploadfile)
            print('File successfully uploaded.')


        elif commands[0] == 'makedir':                                      #### CREATE A DIRECTORY ####
